...
# 💡 글로벌 CSS 설정 (헤더 및 툴바 460px 강제 고정 및 붕괴 방어)
st.markdown(f"""
<style>
    ...
    * {{ animation-duration: 0s !important; transition-duration: 0s !important; }}
    ...
    div[data-testid="stHorizontalBlock"]:has(div[data-baseweb="select"]) {{
        ...
        justify-content: space-between !important; max-width: 460px !important; width: 100% !important; margin: 0 auto 5px 0 !important;
    }}
    div[data-testid="stHorizontalBlock"]:has(div[data-baseweb="select"]) > div[data-testid="column"]:nth-child(1) {{ flex: 1 1 auto !important; min-width: 0 !important; width: auto !important; padding: 0 !important; }}
    div[data-testid="stHorizontalBlock"]:has(div[data-baseweb="select"]) > div[data-testid="column"]:nth-child(2) {{ flex: 0 0 120px !important; min-width: 120px !important; width: 120px !important; padding: 0 !important; }}
    ...
    /* 🚨 2. 툴바 (8개 컬럼) 460px 고정 및 640px 이하 붕괴 완벽 차단 */
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) {{
        display: flex !important; flex-direction: row !important; flex-wrap: nowrap !important; align-items: center !important;
        background-color: {t['top']} !important; border-radius: 6px !important; padding: 4px 2px !important;
        margin-bottom: 10px !important; width: 100% !important; max-width: 460px !important; box-shadow: 0 4px 6px rgba(0,0,0,0.1) !important; gap: 2px !important;
    }}
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) > div[data-testid="column"] {{
        min-width: 0 !important; width: auto !important; padding: 0 !important; margin: 0 !important; flex: 1 1 0% !important;
    }}
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) > div[data-testid="column"]:nth-child(2) {{ flex: 1.5 1 0% !important; }}
    
    /* 툴바 내 버튼들 디자인 (스트림릿 기본 디자인 무력화) */
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) .stButton > button, .tb-html-btn {{
        height: 32px !important; border-radius: 4px !important; font-size: 13px !important; font-weight: bold !important;
        background-color: transparent !important; color: {t['text']} !important; border: none !important; padding: 0 !important;
        line-height: 32px !important; width: 100% !important; min-width: 0 !important; text-align: center; display: block;
        margin: 0 !important; cursor: pointer; user-select: none;
    }}
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) .stButton > button:active, .tb-html-btn:active {{ opacity: 0.6; }}
    div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) div[data-testid="column"] p {{ margin: 0 !important; padding: 0 !important; }}

    /* 🚨 3. 체크박스를 활용한 0초 딜레이 메뉴 접기/펼치기 */
    body:has(#chk-memo:checked) label[for="chk-memo"],
    body:has(#chk-zero:checked) label[for="chk-zero"],
    body:has(#chk-extra:checked) label[for="chk-extra"] {{
        background-color: {t['hl_per']} !important; color: #ffffff !important;
    }}
...
# ---------------------------------------------------------
# 1. 상단 헤더 (이름선택창 포함, 460px 고정 영역)
# (이 영역은 교사를 바꿀 때만 전체 화면이 새로고침 됩니다)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 2. 🔥 대망의 @st.fragment: 로딩 없는 광속 데이터 렌더링 구역
# ---------------------------------------------------------
@st.fragment
def schedule_dashboard():
    # 💡 1. 툴바 버튼 동작시 이곳(fragment) 내부만 최신 데이터로 재호출 됨
    custom_data = {}
    memos_list = []
    try:
    ...
    # '이번주' 버튼 색상 활성화 CSS 조작
    if st.session_state.week_offset == 0:
        st.markdown(f"""<style>
        div[data-testid="stHorizontalBlock"]:has(> div[data-testid="column"]:nth-child(8):last-child) > div[data-testid="column"]:nth-child(2) .stButton > button {{
            background-color: {t['hl_per']} !important; color: #ffffff !important;
        }}
        </style>""", unsafe_allow_html=True)

    # 💡 2. 툴바 구역 (총 8개 컬럼, st.button 활용으로 새로고침 방지)
    c1, c2, c3, c4, c5, c6, c7, c8 = st.columns(8)
    with c1:
        if st.button("◀", use_container_width=True): st.session_state.week_offset -= 1
    with c2:
        if st.button("이번주", use_container_width=True): st.session_state.week_offset = 0
    with c3:
        if st.button("▶", use_container_width=True): st.session_state.week_offset += 1
    with c4:
        # [요청 반영] 전체 페이지 새로고침 없이 CSV 및 DB 데이터만 최신화
        if st.button("🔄", use_container_width=True): load_csv.clear()
    
    # CSS 체크박스 해킹을 위한 HTML 라벨 배치 (0초 반응속도)
    with c5: st.markdown("<label class='tb-html-btn' for='chk-memo'>📝</label>", unsafe_allow_html=True)
    with c6: st.markdown("<label class='tb-html-btn' for='chk-zero'>☀️</label>", unsafe_allow_html=True)
    with c7: st.markdown("<label class='tb-html-btn' for='chk-extra'>🌙</label>", unsafe_allow_html=True)
    
    with c8:
        if st.button("⚙️", use_container_width=True): settings_modal()
    ...
    html_parts = []
    # [요청 반영] 메모장 기본 활성화 상태 적용 (checked)
    html_parts.append("<input type='checkbox' id='chk-memo' style='display:none;' checked='checked' />")
