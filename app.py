...
# 💡 1-1. PWA(웹앱) 설정 및 아이콘 강제 주입 (수정사항 1 반영)
components.html("""
<script>
...
# 💡 글로벌 CSS 설정 (수정사항 3: 구분선 가독성 강화 반영)
st.markdown(f"""
<style>
...
def display_dashboard():
    ...
    active_row_idx = -1  # 색 채우기 대상
    preview_row_idx = -1 # 테두리 대상
    
    # 07:40(조회 시작) 이전이거나 17:50(9교시 끝) 이후는 제외
    for idx, (p_name, t_range) in enumerate(period_times):
        ...
        if s_mins <= now_mins < e_mins:
            active_row_idx = idx
            break
        elif now_mins < s_mins: # 가장 먼저 만나는 미래의 교시
            preview_row_idx = idx
            break
    ...
    for row_idx, (period, time_str) in enumerate(period_times):
        ...
        for col, day in enumerate(days):
            ...
            if is_current_week and col == today_idx:
                if row_idx == active_row_idx: cell_class = "hl-fill-yellow"
                elif row_idx == preview_row_idx: cell_class = "hl-border-yellow"

            font_sz_str, line_height = "14px", "1.2"
            if period == "학사일정":
                font_sz = 12
                if subject:
                    lines = subject.split('\n'); num_lines = len(lines); max_len = max([len(l) for l in lines] if lines else [0])
    ...
    if st.session_state.show_memo:
        ...
        if memos_list:
            ...
            for m in memos_list:
                num, text, is_strike, is_imp = m['display_num'], m['memo_text'], m.get('is_strike', False), m.get('is_important', False)
