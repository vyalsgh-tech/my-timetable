...
<style>
    html, body, .stApp {{
        ...
        font-family: '{st.session_state.font_name}', sans-serif !important;
        color: {t['text']} !important;
    }}
    .block-container {{
        padding: 0.45rem 0.25rem 1rem 0.25rem !important;
    ...
    .controlbar {{
        display: grid;
        grid-template-columns: 42px 1fr 42px 42px 42px 42px 42px;
        gap: 4px;
        width: 100%;
        align-items: center;
        margin-bottom: 8px;
    }}
    .controlbar .stButton > button {{
    ...
    @media (max-width: 480px) {{
        ...
        .controlbar {{
            grid-template-columns: 34px 1fr 34px 34px 34px 34px 34px;
            gap: 3px;
        }}
        .top-title {{
            font-size: 15px;
...
if st.session_state.logged_in_user is None:
    st.markdown("<div style='text-align:center; padding: 2rem 0 1rem 0;'><div style='font-size: 3rem;'>🏫</div><h1 style='font-size: 26px; font-weight: 800;'>명덕외고 뷰어</h1></div>", unsafe_allow_html=True)
            ...
            if new_id and new_pw:
                ...
                if r.status_code == 200 and len(r.json()) > 0:
                    ...
                    if r2.status_code in [200, 201]:
                        st.success("계정 생성 완료! 로그인 탭에서 접속해주세요.")
                    else:
                        st.error("생성 실패.")
    st.stop()
...
top_left, top_right = st.columns([1, 0.35], gap="small")
with top_left:
    st.markdown("<div class='top-title'>🏫 명덕외고 시간표 뷰어</div>", unsafe_allow_html=True)
with top_right:
...
b1, b2, b3, b4, b5, b6, b7 = st.columns([0.7, 1.6, 0.7, 0.7, 0.7, 0.7, 0.7], gap="small")
with b1:
    if st.button("◀", use_container_width=True):
        st.session_state.week_offset -= 1
        st.rerun()
with b2:
    if st.button("이번주", use_container_width=True, type="primary" if st.session_state.week_offset == 0 else "secondary"):
...
with b3:
    if st.button("▶", use_container_width=True):
        ...
        st.rerun()
with b4:
    if st.button("📝", use_container_width=True, type="primary" if st.session_state.show_memo else "secondary"):
        ...
        st.rerun()
with b5:
    if st.button("☀️", use_container_width=True, type="primary" if st.session_state.show_zero else "secondary"):
        ...
        st.rerun()
with b6:
    if st.button("🌙", use_container_width=True, type="primary" if st.session_state.show_extra else "secondary"):
...
with b7:
    with st.popover("⚙️", use_container_width=True):
        ...
        if new_theme != themes[st.session_state.theme_idx]["name"]:
            ...
            st.rerun()
        new_font = st.selectbox("폰트", ["맑은 고딕", "바탕", "돋움", "굴림", "Arial"], index=["맑은 고딕", "바탕", "돋움", "굴림", "Arial"].index(st.session_state.font_name))
    ...
    for col, day in enumerate(days):
        ...
        if is_current_week and col == today_idx:
            if row_idx == active_row:
                td_cell_class = "class='hl-fill-yellow'"
            elif row_idx == preview_row:
                td_cell_class = "class='hl-border-yellow'"
