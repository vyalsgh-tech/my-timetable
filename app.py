...
if "action" in params and params["action"] == "settings":
    st.query_params.clear(); st.query_params["user"] = st.session_state.logged_in_user; settings_modal()
...
# ---------------------------------------------------------
u = st.session_state.logged_in_user; cur_w = st.session_state.week_offset; cur_t = st.session_state.teacher
link_prev = f"/?user={u}&w={cur_w - 1}"; link_next = f"/?user={u}&w={cur_w + 1}"; link_today = f"/?user={u}&w=0"; link_set = f"/?user={u}&w={cur_w}&action=settings"
bg_today = t['hl_per'] if (cur_w == 0) else "transparent"; fg_today = "#ffffff" if (cur_w == 0) else t['text']
...
html_parts = [f"<style>.pure-html-toolbar {{ display: flex; flex-direction: row; flex-wrap: nowrap; align-items: center; background-color: {t['top']}; padding: 4px 2px; border-radius: 6px; margin-bottom: 10px; width: 100%; max-width: 460px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); gap: 2px; }}.tb-btn {{ flex: 1 1 0; text-align: center; text-decoration: none !important; color: {t['text']}; font-size: 13px; font-weight: bold; padding: 8px 0; border-radius: 4px; background-color: transparent; line-height: 1; cursor: pointer; display: block; }}.tb-btn-wide {{ flex: 1.5 1 0; }}.row-zero, .row-extra, #memo-section {{ display: none; }}#chk-zero:checked ~ .app-container .row-zero {{ display: table-row !important; }}#chk-extra:checked ~ .app-container .row-extra {{ display: table-row !important; }}#chk-memo:checked ~ .app-container #memo-section {{ display: block !important; }}#chk-zero:checked ~ .app-container label[for='chk-zero'], #chk-extra:checked ~ .app-container label[for='chk-extra'], #chk-memo:checked ~ .app-container label[for='chk-memo'] {{ background-color: {t['hl_per']} !important; color: #ffffff !important; }}.mobile-table {{ width: 100%; table-layout: fixed; border-collapse: collapse; font-size: 14px; }}.mobile-table th {{ border: 1px solid {t['grid']}; padding: 4px 1px; text-align: center; height: 45px; }}.mobile-table td {{ border: 1px solid {t['grid']}; padding: 0px; text-align: center; vertical-align: middle; height: 65px; word-break: keep-all; font-weight: bold; font-size: 14px; }}.hl-border-red {{ box-shadow: inset 0 0 0 3px {t['hl_per']} !important; z-index: 10; }}.hl-fill-yellow {{ background-color: {t['hl_cell']} !important; color: black !important; }}</style>"]
html_parts.append(f"<input type='checkbox' id='chk-memo' style='display:none;' {chk_memo_attr} /><input type='checkbox' id='chk-zero' style='display:none;' {chk_zero_attr} /><input type='checkbox' id='chk-extra' style='display:none;' {chk_extra_attr} />")
html_parts.append(f"<div class='app-container'><div class='pure-html-toolbar'><a class='tb-btn' href='{link_prev}' target='_self'>◀</a><a class='tb-btn tb-btn-wide' style='background-color:{bg_today}; color:{fg_today};' href='{link_today}' target='_self'>이번주</a><a class='tb-btn' href='{link_next}' target='_self'>▶</a><label class='tb-btn' for='chk-memo'>📝</label><label class='tb-btn' for='chk-zero'>☀️</label><label class='tb-btn' for='chk-extra'>🌙</label><a class='tb-btn' href='{link_set}' target='_self'>⚙️</a></div>")
...
for row_idx, (period, time_str) in enumerate(period_times):
    ...
    html_parts.append("</tr>")
html_parts.append(f"</table></div><div id='memo-section' style='margin-top:10px;'><h3 style='font-size:15px; color:{t['text']};'>📝 {st.session_state.teacher} 메모장</h3><div style='height:300px; overflow-y:auto; border:1px solid {t['grid']}; border-radius:6px; padding:6px;'>")
